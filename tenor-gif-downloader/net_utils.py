...
class InternetUtils:
    ...
    def files_downloader(self, folder_path: str, final_gifs: list):
        """
            Downloads the specified number of files to destination folder
        """

        if os.path.exists(folder_path):
            for each_gif in final_gifs:
                img_resp = requests.get(each_gif, stream=True)
                if img_resp.status_code == 200:
                    gif_index = len(os.listdir(folder_path))
                    with open(f"{folder_path}/{gif_index}.gif", "wb") as file_p:
